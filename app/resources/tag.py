...
class TagsInStore(MethodView):
    ...
    def post(self, tag_data, store_id):
        if TagModel.query.filter(TagModel.store_id == store_id, TagModel.name == tag_data["name"]).first():
            abort(400, message=f"A tag with name {tag_data['name']} already exists in the store.")
...
@blp.route("/tag/<int:item_id>/tag/<string:tag_id>")
class LinkTagsToItem(MethodView):
    ...
    def post(self, item_id, tag_id):
        ...
        try:
            ...
            abort(500, message="An error occurred whiile inserting the tag")
...
class Tag(MethodView):
    ...
    def delete(self, tag_id):
        ...
        abort(400, message="Could not delete tag. Make sure it is not associated with any items, then try again.")